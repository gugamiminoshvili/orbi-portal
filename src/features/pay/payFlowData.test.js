...
// Two apartments in "Alpha", one in "Beta". Positive = owed, negative = paid
// ahead (utils/balance.js) — owedFor reads the balance straight through.
const ROW1 = {
  code: 'C1',
  epcode: 'EP1',
...
const ROW2 = {
  code: 'C2',
  epcode: 'EP2',
  electricity: -30, // in advance
  internet: { balance: -5, balanceWithPenalty: -5, cost: 10, penalty: 0 }, // in advance
  maintenance: 0, // zero
}
const ROW3 = {
  code: 'C3',
...
describe('owedFor', () => {
  test('electricity/internettv are read straight off (already GEL, no sign flip)', () => {
  ...
  test('zero balance is treated as non-owed (owed <= 0)', () => {
    // toBeCloseTo rather than toBe: the USD branch multiplies by the rate,
    // and 0 * 2.5 can surface as -0 depending on the input's sign.
...
describe('buildComplexes', () => {
  test('groups by project and sums unpaid bills / outstanding GEL across all 3 utilities', () => {
    ...
    // row2: all advance/zero = 0 owed bills
